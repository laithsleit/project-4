...
if ($_SERVER['REQUEST_METHOD'] == 'POST') {
    ...
    if (!empty($data)) {
        $query = 'INSERT INTO products(image, name, description, price, price_after_discount, category_id, section, created_at) VALUES (?,?,?,?,?,?,?,NOW());';

        $stmt = $pdo->prepare($query);

        $success = $stmt->execute([$data['image'], $data['name'], $data['description'], $data['price'], $data['price_after_discount'], $data['category_id'], $data['section']]);
